...
How well did the polls predict the margin between Trump and Clinton in the popular vote? The following plot shows the distribution of projected margins from national-level polls conducted in the two weeks prior to Election Day (n = 363). In the final popular-vote tally, Clinton beat Trump by a margin of 1.97% (more than 2.6 million votes). The projected margin (estimated from the average of the polls in the final two weeks) was 2.18%. The polls appeared to do pretty well here -- the projected margin was not significantly different from the final (actual) margin.
...
```{r errors, echo=FALSE, message=FALSE, fig.align="center"}
...
Visually, it appears that roughly the same number of polls overestimated Clinton and or Trump (the density plots for both candidates have roughly the same area within the range of negative errors). However, it seems that the largest number of polls underestimating Clinton did so by around 3--4 percentage points (notice the peak of Clinton's density plot), while more polls that underestimated Trump did so by larger amounts (notice the fatter tail of Trump's plot, indicating positive skew in the errors).

From this sample of polls in the two weeks immediately preceeding Election day, national-level polls were generally accurate about the margin between the candidates (projecting ~2% lead for Clinton), but underestimated the vote-shares of both candidates. Polls underestimated Clinton mostly by about 3--4 percentage points, while there was more variability in how much they underestimated Trump. Quite a few polls underestimated him by quite a bit -- by more than 5 percentage points!
...
* The majority of polls underestimated Trump, except for 11 states that either got the margin right (New York) or underestimated Clinton (Illinois, Washington, Massachusetts, Connecticut, Nevada, Rhode Island, California, Hawaii, and DC).
* Looking only at states that Clinton won, polling error was distributed quite symmetrically -- she was overestimated in some states, underestimated in others. This is what you'd expect for both candidates and the set of states they won if there's nothing systematically wrong with the polls.
...
As stated above, there's been a ton of analysis about how the polls missed Trump's win. This post has avoided those issues, except to show that there are specific ways that the polls did and did not get things right. This started mainly as an exercise in [scraping election results](https://github.com/kevinsoo/polls2016/blob/master/scrape_results.R) (though I suppose in time they would all have been released in an easily-accessible form). I'm planning to write more on the data presented here -- I would love to talk if you have any ideas for interesting analyses!